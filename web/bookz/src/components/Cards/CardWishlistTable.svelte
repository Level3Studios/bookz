<script lang="ts">
  ...
  let wishlistBooks = viewModel.storedWishlistBooks;

  // interactions
  function deleteBook(book: Item) {
    viewModel.removeBookFromWishlist(book);
    wishlistBooks = wishlistBooks.filter((item) => item.id !== book.id);
  }
...
>
  <div class="rounded-t mb-0 px-4 py-3 border-0">
    <div class="flex flex-wrap items-center">
      <div class="relative w-full px-4 max-w-full flex-grow flex-1">
        <h3 class="font-semibold text-lg text-slate-700">My Wishlist</h3>
      </div>
    </div>
  </div>
  <div class="block w-full overflow-x-auto">
    ...
    <table class="items-center w-full bg-transparent border-collapse">
      ...
      <tbody>
        {#await wishlistBooks then result}
          {#each result as book}
            <tr>
              <th
                ...
                <ul class="list-outside">
                  ...
                  <li>
                    <button
                      ...
                      type="button"
                    >
                      <span><i class="fas fa-book mr-2" /> Move to Library</span
                      >
                    </button>
                  </li>
                </ul>
