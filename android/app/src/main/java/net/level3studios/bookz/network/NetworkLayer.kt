...
object NetworkLayer {
    ...
    fun searchForBooks(searchTerm: String, maxResults: Int) {
        ...
        query.httpGet().responseObject(BooksResponseDeserializer()) { _, _, result ->
            when (result) {
                ...
                is Result.Success -> {
                    val (data, _) = result
                    data?.let { Log.i("Success Books found:", "{${it.items.count()}}") }
                }
            }
        }
    }
    ...
    class BooksResponseDeserializer: ResponseDeserializable<BooksResponse> {
        override fun deserialize(reader: Reader): BooksResponse = Gson().fromJson(reader, BooksResponse::class.java)
    }
}